...
signed main() {
  ...
  cin >> f;
  sort(f.begin(), f.end());
  vector<long long> sum(f.size(), 0);
  sum[0] = f[0];
  for (long long i = 0; i < f.size() - 1; i++) sum[i + 1] = sum[i] + f[i + 1];
  long long ans = p * RUD(n, d);
  for (long long i = 0; i < RUD(n, d); i++) ans = min(ans, sum[n - 1 - (i * d)] + (p * i));
  cout << ans << endl;
  return 0;
}
#else
