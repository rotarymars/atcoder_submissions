...
signed main() {
  ...
  auto pairs = [](map<long long, long long> &m) -> long long {
    long long ret = 0;
    for (auto &p : m)
      ret += p.second * (p.second - 1) / 2;
    return ret;
  };
  map<long long, long long> mAB, mBC, mAC, mT;
  long long base = 2ll * n + 1;
  for (int i = 0; i <= n; ++i) {
    long long ab = a[i] - b[i];
    long long bc = b[i] - c[i];
    long long ac = a[i] - c[i];
    ++mAB[ab];
    ++mBC[bc];
    ++mAC[ac];
    ++mT[(ab + n) * base + (bc + n)];
  ...
  long long bad = pairs(mAB) + pairs(mBC) + pairs(mAC) - 2 * pairs(mT);
  io.writeln(total - bad);
  return 0;
}
