...
int h, w;
FastIO io;
void printv(const vector<vector<bool>> &v) {
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      io.write(v[i][j] ? "#" : ".");
    }
...
signed main() {
  ...
  int dh[] = {-1, -1, 0, 1, 1, 1, 0, -1}, dw[] = {0, 1, 1, 1, 0, -1, -1, -1};
  ...
  for (int i = 0; i < h; ++i)
    for (int j = 0; j < w; ++j) {
      ...
      for (int k = 0; k < 8; ++k) {
        ...
        if (0 <= ni && ni < h && 0 <= nj && nj < w && !v[ni][nj]) {
          dist[i][j] = 0;
          bfs.push_back({i, j});
          break;
        }
  ...
  for (size_t head = 0; head < bfs.size(); ++head) {
    int i = bfs[head].first, j = bfs[head].second;
    for (int k = 0; k < 8; ++k) {
      int ni = i + dh[k], nj = j + dw[k];
