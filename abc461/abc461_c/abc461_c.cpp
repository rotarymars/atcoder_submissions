...
signed main() {
  ...
  io.read(v);
  sort(v.begin(), v.end(), [](pair<int, int> p, pair<int, int> pp) {
    return p.second > pp.second;
  });
  list<pair<int, int>> li(v.begin(), v.end());
  set<int> se;
  auto it = li.begin();
  long long val = 0;
  while (se.size() < m) {
    if (!se.contains(it->first)) {
      se.insert(it->first);
      val += it->second;
      it = li.erase(it);
    } else
  ...
  int left = k - m;
  v = vector(li.begin(), li.end());
  for (auto it = v.begin(); it != v.end() && left > 0; ++it) {
    val += it->second;
    left--;
  }
  io.writeln(val);
