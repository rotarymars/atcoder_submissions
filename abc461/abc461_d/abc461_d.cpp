...
size_t calcsum(const vector<vector<int>> &sum, int i, int j, int sizeh,
               int sizew) {
  return sum[i + sizeh][j + sizew] - sum[i][j + sizew] - sum[i + sizeh][j] +
         sum[i][j];
}
using namespace std;
signed main() {
  ...
  io.read(v);
  vector<vector<int>> sum(1 + h, vector<int>(1 + w, 0));
  sum[0][0] = 0;
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      sum[i + 1][j + 1] =
          sum[i][j + 1] + sum[i + 1][j] - sum[i][j] + (v[i][j] == '1');
    }
  }
#define _DEBUG 0
#if _DEBUG
...
#endif
  long long cnt = 0;
  for (int sizeh = 1; sizeh <= h; ++sizeh) {
    for (int sizew = 1; sizew <= w; ++sizew) {
      for (int i = 0; i < h - sizeh + 1; ++i) {
        for (int j = 0; j < w - sizew + 1; ++j) {
#if _DEBUG
          io.writeln(calcsum(sum, i, j, sizeh, sizew));
#endif
          if (calcsum(sum, i, j, sizeh, sizew) == k)
            ++cnt;
        }
      }
    }
