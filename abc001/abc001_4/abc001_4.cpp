...
signed main() {
  ...
  int n;
  io.read(n);
  vector<pair<Time, Time>> v(n);
  for (auto &i : v) {
    char a, b, c, d;
    io.read(a), io.read(b), io.read(c), io.read(d);
    i.first.hour = (a - '0') * 10 + (b - '0');
    i.first.minute = (c - '0') * 10 + (d - '0');
    io.read(a);
    io.read(a), io.read(b), io.read(c), io.read(d);
    i.second.hour = (a - '0') * 10 + (b - '0');
    i.second.minute = (c - '0') * 10 + (d - '0');
  }
  for (auto &i : v) {
    i.first.minute /= 5;
    i.first.minute *= 5;
    if (i.second.minute % 5) {
      i.second.minute /= 5;
      i.second.minute *= 5;
      i.second.minute += 5;
      if (i.second.minute == 60) {
        i.second.minute = 0;
        i.second.hour++;
      }
    }
  ...
  for (auto &i : ans) {
    if (i.first.hour < 10) {
      io.write('0');
      io.write(i.first.hour);
    } else {
      io.write(i.first.hour);
    }
    if (i.first.minute < 10) {
      io.write('0');
      io.write(i.first.minute);
    } else {
      io.write(i.first.minute);
    }
    io.write('-');
    if (i.second.hour < 10) {
      io.write('0');
      io.write(i.second.hour);
    } else {
      io.write(i.second.hour);
    }
    if (i.second.minute < 10) {
      io.write('0');
      io.write(i.second.minute);
    } else {
      io.write(i.second.minute);
    }
    io.write('\n');
