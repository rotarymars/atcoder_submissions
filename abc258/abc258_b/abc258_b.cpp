...
signed main() {
  ...
  VI gh{ -1, -1, 0, 1, 1, 1, 0, -1 }, gw{ 0,1,1,1,0,-1,-1,-1 };
  LL ans = 0;
#ifdef _DEBUG
  for (auto& i : a)
  {
...
#endif // _DEBUG
  for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) for (int k = 0; k < 8; k++) {
    pair nowco{ i, j };
    LL temp = 0;
    for (int l = 0; l < n; l++) {
      temp *= 10;
      temp += a[nowco.first][nowco.second] - '0';
      nowco.first += gh[k];
      if (nowco.first < 0) nowco.first += n;
      nowco.second += gw[k];
      if (nowco.second < 0) nowco.second += n;
      nowco.first %= n, nowco.second %= n;
    }
    /*temp *= 10;
    temp += a[nowco.first][nowco.second] - '0';*/
    ans = max(ans, temp);
