...
int main() {
  ...
  VI horizontalsum(n), verticalsum(n);
  ...
  LL ans = 0;
  //PRINT1D(verticalsum), PRINT1D(horizontalsum);
  for (size_t i = 0; i < n; i++)
  {
    for (size_t j = 0; j < n; j++)
    {
      if (grid[i][j] == 'o')
      {
        //cout << i << " " << j << " " << (verticalsum[i] - 1) * (horizontalsum[j] - 1) << "\n";
        ans += (verticalsum[i] - 1) * (horizontalsum[j] - 1);
      }
    }
  }
  cout << ans << "\n";
