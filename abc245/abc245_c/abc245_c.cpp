...
signed main() {
  ...
  cin >> a >> b;
  vector<bool> dp(n, false), dpp(n, false); //dp : a, dpp : b
  dp[0] = dpp[0] = true;
  for (int i = 1; i < n; i++) {
    if (dp[i - 1] && abs(a[i] - a[i - 1]) <= k) dp[i] = true;
    if (dpp[i - 1] && abs(a[i] - b[i - 1]) <= k) dp[i] = true;
    if (dpp[i - 1] && abs(b[i] - b[i - 1]) <= k) dpp[i] = true;
    if (dp[i - 1] && abs(b[i] - a[i - 1]) <= k) dpp[i] = true;
    DPln(dp); DPln(dpp);
  }
  if (dp.back() || dpp.back()) Yes();
  else No();
  return 0;
}
#else
