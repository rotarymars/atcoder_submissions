...
signed main() {
  ...
  VB dp1(n, false), dp2(n, false);
  dp1[0] = dp2[0] = true;
  for (int i = 1; i < n; i++) {
    if (dp1[i - 1] && abs(a[i - 1] - a[i]) <= k) dp1[i] = true;
    if (dp1[i - 1] && abs(a[i - 1] - b[i]) <= k) dp2[i] = true;
    if (dp2[i - 1] && abs(b[i - 1] - b[i]) <= k) dp2[i] = true;
    if (dp2[i - 1] && abs(b[i - 1] - a[i]) <= k) dp1[i] = true;
  }
  if (dp1[n - 1] || dp2[n - 1]) Yes();
  else No();
  return 0;
}
#else
