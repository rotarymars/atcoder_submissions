...
signed main()
{
	...
	int one, five, ten, fifty, hundred, fivehundred;
	cin >> one >> five >> ten >> fifty >> hundred >> fivehundred;
	int n;
	cin >> n;
	for (; n; n--)
	{
		int tmp;
		cin >> tmp;
		while (tmp >= 500 && fivehundred >= 1)
		{
			tmp -= 500;
			fivehundred--;
		}
		while (tmp >= 100 && hundred >= 1)
		{
			tmp -= 100;
			hundred--;
		}
		while (tmp >= 50 && fifty >= 1)
		{
			tmp -= 50;
			fifty--;
		}
