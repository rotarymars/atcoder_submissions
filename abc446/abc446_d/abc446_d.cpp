...
signed main() {
  ...
  function<int(int)> compute = [&](int idx) -> int {
    if (computed[idx] != -1) {
      return computed[idx];
    }
    if (idx == 0) {
    ...
    auto it = mp.find(v[idx] - 1);
    if (it == mp.end()) {
      return computed[idx] = 1;
    }
    auto itt = lower_bound(it->second.begin(), it->second.end(), idx);
    if (itt == it->second.begin()) {
      return computed[idx] = 1;
    }
    --itt;
    return computed[idx] = compute(*itt) + 1;
  };
  for (int i = n - 1; i >= 0; --i) {
    compute(i);
  }
