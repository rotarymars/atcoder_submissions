...
signed main() {
  ...
  cin >> n >> m;
  DPln(RUD(m, 4));
  for (int i = 0; i <= RUD(m, 4); i++) {
    int newn = n - i, newm = m - 4 * i;
    DPln(i); DPln(newn); DPln(newm);
    if (2 * newn <= newm && (newm - 2 * newn) <= newn && (newm - 2 * newn) >= 0) {
      cout << 3 * newn - newm << " " << (newm - 2 * newn) << " " << i << endl;
      return 0;
    }
  }
  cout << "-1 -1 -1\n";
