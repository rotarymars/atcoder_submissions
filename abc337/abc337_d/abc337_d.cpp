...
int main()
{
  ...
  for (size_t i = 0; i < h; i++)
  {
    ...
    for (size_t j = 1; j <= w; j++)
    {
      if (a[i][j - 1] == '.')
      {
        sidedot[j] = sidedot[j - 1] + 1;
        sidex[j] = sidex[j - 1];
      }
      else if (a[i][j - 1] == 'x')
      {
        sidex[j] = sidex[j - 1] + 1;
        sidedot[j] = sidedot[j - 1];
      }
      else
      {
        sidex[j] = sidex[j - 1];
    ...
    for (int j = 0; j <= (w - k); j++)
    {
      if (sidex[j + k] - sidex[j] == 0)
      {
        ans = min(ans, sidedot[j + k] - sidedot[j]);
      }
    }
  }
  VI hordot(h + 1, 0), horx(h + 1, 0);
  for (size_t i = 0; i < w; i++)
  {
    ...
    for (int j = 0; j <= (h - k); j++)
    {
      if (horx[j + k] - horx[j] == 0)
      {
        ans = min(ans, hordot[j + k] - hordot[j]);
      }
    }
  }
  if (ans == INT_MAX)
