...
signed main()
{
    ...
    que.push(move(make_pair(s,0)));
    while(true){
        auto tmppair=que.front();
        auto tmp=tmppair.first;
        que.pop();
        if(tmp=="atcoder"s){
        ...
        for(int i=0;i<6;i++){
            ...
            if(!se.contains(tmps)){
                que.push(move(make_pair(tmps,tmppair.second+1)));
                se.insert(tmps);
            }
        }
    }
...
void VSTOVCC(vector<string> &a, vector<vector<char>> &b)
{
...
void STOVC(string &a, vector<char> &b)
{
    for (size_t i = 0; i < a.size(); i++)
    {
        b[i] = a[i];
