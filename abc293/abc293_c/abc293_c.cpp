...
signed main() {
  ...
  cin >> h >> w;
  VVI a(h, VI(w));
  for (VI& i : a) for (int& j : i) cin >> j;
  LL maxi = 0;
  VI permutation(h + w - 2);
  for (int i = 0; i < h - 1; i++) {
    permutation[i] = 0;
  }
  for (int i = h - 1; i < h + w - 2; i++) permutation[i] = 1;
  int ans = 0;
  do {
    SI already;
    PII nowco{ 0, 0 };
    already.insert(a[0][0]);
    bool ok = true;
    for (int i = 0; i < h + w - 2; i++) {
      if (permutation[i]) nowco.second++;
      else nowco.first++;
      if (already.contains(a[nowco.first][nowco.second])) {
        ok = false;
        break;
      }
      already.insert(a[nowco.first][nowco.second]);
    }
    if (ok) ans++;
