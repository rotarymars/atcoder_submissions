...
signed main() {
  ...
  io.read(v);
  vector<vector<pair<int, int>>> minp(
      h, vector<pair<int, int>>(w, {-1, -1}));     // 0,1
  queue<pair<pair<int, int>, pair<int, int>>> que; //{{h,w},{door,cnt}}};
  int sh, sw;
  for (int i = 0; i < h; ++i)
    for (int j = 0; j < w; ++j) {
      if (v[i][j] == 'S') {
  ...
  while (!que.empty()) {
    auto now = que.front();
    que.pop();
    const int ci = now.first.first, cj = now.first.second;
    const int p = now.second.first % 2, cnt = now.second.second;
    if (p == 0) {
      if (minp[ci][cj].first != -1)
        continue;
      minp[ci][cj].first = cnt;
    } else {
      if (minp[ci][cj].second != -1)
        continue;
      minp[ci][cj].second = cnt;
    }
    if (v[ci][cj] == 'G') {
      io.writeln(cnt);
      return 0;
    }
    for (int k = 0; k < (int)(sizeof(dh) / sizeof(int)); ++k) {
      const int nh = ci + dh[k], nw = cj + dw[k];
      if (nh < 0 || nh >= h || nw < 0 || nw >= w)
        continue;
      const char c = v[nh][nw];
      if (c == '#')
        continue;
      else if (c == '.' || c == 'S' || c == 'G' || c == (p == 0 ? 'o' : 'x')) {
        que.push({{nh, nw}, {p, cnt + 1}});
      } else if (c == '?') {
        que.push({{nh, nw}, {p ^ 1, cnt + 1}});
      }
    }
  }
  io.writeln(-1);
