...
namespace {
  ...
  pair<int, int> NUMTOCO(const int n, const int w) {
    return { n / w, n % w };
  }
...
void PRINTPII(vector<pair<int, int>> a) {
  for (size_t i = 0; i < a.size(); i++)
  {
...
int main()
{
  ...
  cin >> n >> q;
  //cout << n << " " << q << "\n";
  vector<pair<int, int>> trajectory(n);
  //coordinate real, no --
  ...
  for (size_t i = 0; i < q; i++)
  {
    ...
    {
      cin >> qc;
      pair<int, int> nowtraj = trajectory[trajectory.size() - 1];
      if (qc == 'R')
      {
        trajectory.push_back({ nowtraj.first + 1, nowtraj.second });
      }
      else if (qc == 'L')
      {
        trajectory.push_back({ nowtraj.first - 1, nowtraj.second });
      ...
      {
        trajectory.push_back({ nowtraj.first, nowtraj.second + 1 });
      }
      else
      {
        trajectory.push_back({ nowtraj.first, nowtraj.second - 1 });
    ...
    {
      cin >> p;
      cout << trajectory[trajectory.size() - p].first << " " << trajectory[trajectory.size() - p].second << "\n";
    }
  }
  return 0;
}