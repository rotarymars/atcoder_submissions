...
signed main() {
  ...
  while (t--) {
    ...
    io.read(a, b);
    vector<char> st, tt;
    st.reserve(a.size()), tt.reserve(b.size());
    for (int i = 0; i < a.size(); ++i) {
      st.push_back(a[i]);
      if (*prev(st.end(), 4) == '(' && *prev(st.end(), 3) == 'x' &&
          *prev(st.end(), 2) == 'x' && *prev(st.end(), 1) == ')') {
        st.pop_back();
        st.pop_back();
        st.pop_back();
        st.pop_back();
    ...
    for (int i = 0; i < b.size(); ++i) {
      tt.push_back(b[i]);
      if (*prev(tt.end(), 4) == '(' && *prev(tt.end(), 3) == 'x' &&
          *prev(tt.end(), 2) == 'x' && *prev(tt.end(), 1) == ')') {
        tt.pop_back();
        tt.pop_back();
        tt.pop_back();
        tt.pop_back();
