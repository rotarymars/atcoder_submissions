...
signed main() {
  ...
  int twomax = 0, twomin = INT_MAX, threemax = 0, threemin = INT_MAX, other = 0, threesum = 0, twosum = 0;
  for (size_t i = 0; i < n; i++)
  {
    ...
    twomax = max(twomax, tmptwo);
    twomin = min(twomin, tmptwo);
    twosum += tmptwo;
    int tmpthree = 0;
    while (tmp % 3 == 0)
    {
    ...
    threemax = max(threemax, tmpthree);
    threemin = min(threemin, tmpthree);
    threesum += tmpthree;
    //cout << tmp << endl;
    if (i == 0)
    {
  ...
  cout << twosum + threesum - twomin * n - threemin * n << endl;
  return 0;
}
#else
