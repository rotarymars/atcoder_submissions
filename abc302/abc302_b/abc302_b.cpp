...
string getline(VS& a, int hway, int wway, int hfirst, int wfirst) {
  string ans = "";
  for (size_t i = 0; i < 5; i++)
  {
    ans += a[hfirst + hway * i][wfirst + wway * i];
  }
  return ans;
}
...
int main()
{
  ...
  VI hgrid = { -1, 0, 0, 1, -1, -1, 1, 1};
  VI wgrid = { 0, -1, 1, 0, -1, 1, -1, 1};
  for (int i = 0; i < h; i++)
  {
    for (int j = 0; j < w; j++)
    {
      for (int k = 0; k < hgrid.size(); k++)
      {
        if (INRANGE(0, h - 1, i + hgrid[k] * 4) && INRANGE(0, w - 1, j + wgrid[k] * 4))
        {
          //cout << getline(a, hgrid[k], wgrid[k], i, j) << "\n";
          if (getline(a, hgrid[k], wgrid[k], i, j) == "snuke")
          {
            for (size_t l = 0; l < 5; l++)
            {
              cout << i + hgrid[k] * l + 1 << " " << j + wgrid[k] * l + 1 << "\n";
            }
            return 0;
          }
          /*
