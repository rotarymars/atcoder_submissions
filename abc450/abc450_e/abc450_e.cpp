...
#include <utility>
string x, y;
vector<pair<vector<long long>, long long>> memo;
vector<vector<long long>> prefx, prefy;

constexpr long long LIM = (1LL << 62);

void init_base() {
  prefx.assign(26, vector<long long>(x.size() + 1, 0));
  for (size_t i = 0; i < x.size(); ++i) {
    for (int ch = 0; ch < 26; ++ch)
      prefx[ch][i + 1] = prefx[ch][i];
    prefx[x[i] - 'a'][i + 1]++;
  }
  prefy.assign(26, vector<long long>(y.size() + 1, 0));
  for (size_t i = 0; i < y.size(); ++i) {
    for (int ch = 0; ch < 26; ++ch)
      prefy[ch][i + 1] = prefy[ch][i];
    prefy[y[i] - 'a'][i + 1]++;
  }
}

void expand() {
  auto [v1, s1] = memo[memo.size() - 1];
  auto [v2, s2] = memo[memo.size() - 2];
  vector<long long> v(26, 0);
  for (int i = 0; i < 26; ++i) {
    v[i] = (v1[i] >= LIM - v2[i] ? LIM : v1[i] + v2[i]);
  }
  long long ns = (s1 >= LIM - s2 ? LIM : s1 + s2);
  memo.push_back({v, ns});
}
long long compute(long long n, char c) {
  if (n <= 0)
    return 0;
  if (memo.size() < 2) {
    vector<long long> v(26, 0);
    ...
    memo.push_back({v, x.size()});
    v = vector<long long>(26, 0);
    for (char ch : y) {
      v[ch - 'a']++;
    }
    memo.push_back({v, y.size()});
    init_base();
  }
  while (memo.back().second < n) {
    expand();
  }
  int idx = (int)memo.size() - 1;
  int ci = c - 'a';
  long long ret = 0;
  while (n > 0) {
    if (idx == 0) {
      ret += prefx[ci][min<long long>(n, x.size())];
      break;
    }
    if (idx == 1) {
      ret += prefy[ci][min<long long>(n, y.size())];
      break;
    }
    long long left_len = memo[idx - 1].second;
    if (n <= left_len) {
      --idx;
    } else {
      ret += memo[idx - 1].first[ci];
      n -= left_len;
      idx -= 2;
    }
  }
...
signed main() {
  ...
  for (; q--;) {
    ...
    char c;
    io.read(l, r, c);
    io.writeln(compute(r, c) - compute(l - 1, c));
  }
  return 0;
}
    ...
    if (len == 1) {
      ...
      for (int s = 0; s < (1 << (len - 1)); s++) {
        ...
        for (int i = 0; i < p; i++) {
          ...
          a[i + offset] = l + r;
          a[i + offset + p] =
              (unsigned long long)((unsigned int)(l.val() - r.val()) +
                                   mint::mod()) *
              irot.val();
          ;
        }
