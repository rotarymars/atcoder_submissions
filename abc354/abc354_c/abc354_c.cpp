...
class cl{
  public:
  int index,a,c;
  cl(int d, int b, int e):index(d), a(b), c(e){};
  cl(){};
  void operator=(cl a){
    this->index=a.index;
    this->a=a.a;
...
signed main()
{
  ...
  vector<cl> v(n);
  for (int i = 0; i < n; i++){
    int tmp,temp;cin>>tmp>>temp;
    v[i]=cl(i,tmp,temp);
  }
  vector<cl> vacopy, vccopy;
  vacopy=vccopy=v;
  sort(vacopy.begin(), vacopy.end(), cmp);
  sort(vccopy.begin(), vccopy.end(), comp);
  set<int> eliminated;
  int smallestcost=INT_MAX;
  for (auto i=vacopy.rbegin(); i != vacopy.rend(); i++){
    if (i->c > smallestcost){
      eliminated.insert(i->index);
    }else smallestcost=i->c;
  }
  int biggestattack=0;
  for (auto i=vccopy.rbegin(); i != vccopy.rend(); i++){
    if (i->a < biggestattack){
      eliminated.insert(i->index);
    }else biggestattack=i->a;
  }
  cout << n-eliminated.size() << "\n";
  bool first=true;
  for (int i=0;i<n;i++){
