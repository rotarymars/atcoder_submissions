...
signed main() {
  ...
  string s;
  io.read(s);
  vector<long long> accum(s.size(), 0);
  long long sum = 0;
  vector<long long> chars(26, 0);
  constexpr long long mod = 998244353;
  for (int i = s.size() - 1; i >= 0; --i) {
    if (s.size() - 1 == i)
      accum[s.size() - 1] = 1, sum = 1, chars[s.back() - 'a'] += 1;
    else {
      long long t = 1;
      t += sum - chars[s[i] - 'a'];
      t = (t % mod + mod) % mod;
      accum[i] = t, sum += t, chars[s[i] - 'a'] += t;
      accum[i] %= mod, sum %= mod, chars[s[i] - 'a'] %= mod;
    }
  }
  //   io.writeln(accum);
  io.writeln(accumulate(accum.begin(), accum.end(), 0ll) % mod);
  return 0;
}
